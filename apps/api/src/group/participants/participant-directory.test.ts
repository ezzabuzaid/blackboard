...
function createDirectory(
  ...
  return new ParticipantDirectory({
    directory: resolve(root, 'participants'),
    builtinsDirectory,
...
async function writeParticipant(
  ...
  const writable = directory.mounts(userId).find(({ readOnly }) => !readOnly);
  assert.ok(writable);
  const participant = resolve(writable.hostPath, folder);
  await mkdir(participant, { recursive: true });
  await Promise.all([
    writeFile(resolve(participant, 'identity.json'), JSON.stringify({ name })),
    writeFile(resolve(participant, 'SOUL.md'), 'Be candid and concise.\n'),
    writeFile(resolve(participant, 'AGENTS.md'), 'Own customer evidence.\n'),
    writeFile(resolve(participant, 'MEMORY.md'), '# Memory\n'),
  ]);
}

test('repository and user directories form one participant roster', async () => {
...
test('participant directories isolate users and their model defaults', async () => {
  const root = await mkdtemp(join(tmpdir(), 'baseera-participants-'));
  const builtins = resolve(root, 'builtins');
...
test('a participant folder requires a valid identity file', async () => {
  ...
  try {
    ...
    const writable = directory
      .mounts('user-1')
      .find(({ readOnly }) => !readOnly);
    assert.ok(writable);
    const maya = resolve(writable.hostPath, 'maya');
    await mkdir(maya);
    ...
    await writeFile(resolve(maya, 'identity.json'), 'not json');
    await assert.rejects(
      directory.participants('user-1'),
      /maya.*invalid identity\.json/,
    );
    await writeFile(resolve(maya, 'identity.json'), '{}');
    await assert.rejects(
      directory.participants('user-1'),
...
test('catalog selection mounts only the requested read-only participants', async () => {
  ...
  try {
    ...
    const mounts = directory.mounts('user-1', ['paul-graham']);
    const participants = await directory.participants('user-1', [
      'paul-graham',
    ]);
    ...
    assert.deepEqual(mounts, [
      {
        guestPath: '/workspace/participants/paul-graham',
        hostPath: resolve(catalog, 'paul-graham'),
        readOnly: true,
      },
    ]);
    assert.deepEqual(
